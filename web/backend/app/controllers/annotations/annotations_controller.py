...
@router.get(
    "/features",
    summary="Get Annotation Features",
    description="Retrieve all available annotation features.",
)
def get_annotation_features(db: Session = Depends(get_db)):
    """
    Get all annotation features from the database.

    Args:
        db (Session): The database session.

    Returns:
        dict: List of annotation features.
    """
    # Retrieve all annotation features from the database
    annotation_features = db.query(AnnotationFeatureModel).filter_by(is_active=True).all()

    # Format response data
    data = [
        {
            "id": annotation_feature.id,
            "name": annotation_feature.name,
            "code_name": annotation_feature.name,  # Assuming "code_name" is equivalent to "name"
            "description": annotation_feature.description,
            "logo_url": annotation_feature.logo_url,
        }
    ...
    return standard_response(
        status="success",
        status_code=200,
        message_code="annotation_features_retrieved",
        data=data,
    )
...
@router.get(
    ...
    summary="Get Annotations by Feature",
    description="Retrieve annotations by their feature.",
)
def get_annotations_by_feature(feature_name: str, db: Session = Depends(get_db)):
    """
    Get annotations filtered by their feature.

    Args:
        feature_name (str): The feature name to filter by.
        db (Session): Database session dependency.

    Returns:
        dict: List of annotations matching the feature.

    Raises:
        HTTPException: If no annotations are found for the specified feature.
    """
    # Retrieve the annotation feature from the database
    annotation_feature_entry = (
        db.query(AnnotationFeatureModel)
        .filter(AnnotationFeatureModel.name == feature_name)
        .first()
    )

    if not annotation_feature_entry:
        raise HTTPException(
            status_code=400,
            detail=f"Invalid annotation feature: {feature_name}",
        )

    # Query annotations by feature ID
    annotations = (
        db.query(AnnotationProjectModel)
        .filter(AnnotationProjectModel.annotation_feature_id == annotation_feature_entry.id)
        .all()
    )
    ...
    if not annotations:
        raise HTTPException(
            status_code=404,
            detail=f"No annotations found for feature '{feature_name}'.",
        )
    ...
    return standard_response(
        status="success",
        status_code=200,
        message_code="annotations_by_feature_retrieved",
        data=[
            {
                "id": annotation.id,
                ...
                "annotation_feature": annotation_feature_entry.name,  # Use human-readable name
