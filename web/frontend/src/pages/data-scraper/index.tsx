import React, { useState, useEffect } from "react";
import Layout from "@/components/Layout";
import {
  FaSpider,
  FaFileDownload,
  FaSearch,
  FaDatabase,
  FaCogs,
  FaChevronRight,
  FaChevronLeft,
} from "react-icons/fa";
import { HomeIcon, FolderIcon } from "@heroicons/react/24/outline";
import { useSelector } from "react-redux";
import { RootState } from "@/store/store";
import { useRouter } from "next/router";

const DataScraperPage = () => {
  const { menu } = useSelector((state: RootState) => state.menu);
  const router = useRouter();

  const features = [
    { name: "Web Scraping", icon: FaSpider },
    { name: "Data Crawling", icon: FaSearch },
    { name: "Data Export", icon: FaFileDownload },
    { name: "Database Integration", icon: FaDatabase },
    { name: "Advanced Options", icon: FaCogs },
  ];

  const subFeatures: Record<
    | "Web Scraping"
    | "Data Crawling"
    | "Data Export"
    | "Database Integration"
    | "Advanced Options",
    string[]
  > = {
    "Web Scraping": [
      "Scrape HTML Elements",
      "Extract Data from Tables",
      "Scrape Images and Media",
      "Pagination Handling",
    ],
    "Data Crawling": [
      "Crawl Multiple Pages",
      "Crawl Entire Websites",
      "Custom URL Patterns",
      "Rate Limiting and Throttling",
    ],
    "Data Export": [
      "Export to CSV",
      "Export to JSON",
      "Export to XML",
      "Custom Export Templates",
    ],
    "Database Integration": [
      "Save Data to SQL Databases",
      "Save Data to NoSQL Databases",
      "Real-Time Database Sync",
      "Data Backup Options",
    ],
    "Advanced Options": [
      "Proxy Support",
      "Authentication Handling",
      "Custom Headers and Cookies",
      "JavaScript Rendering with Puppeteer",
    ],
  };

  const [selectedFeature, setSelectedFeature] = useState<keyof typeof subFeatures>(
    "Web Scraping"
  );
  const [isSidebarMinimized, setIsSidebarMinimized] = useState(false);

  useEffect(() => {
    if (router.pathname === "/data-scraper") {
      setSelectedFeature("Web Scraping");
    }
  }, [router.pathname]);

  const breadcrumbItems = [
    { label: "Home", href: "/", icon: <HomeIcon className="w-4 h-4" /> },
    { label: "Data Scraper", href: "/data-scraper", icon: <FolderIcon className="w-4 h-4" /> },
    ...(selectedFeature
      ? [
          {
            label: selectedFeature,
            href: "",
            icon: <FolderIcon className="w-4 h-4" />,
            isActive: true,
          },
        ]
      : []),
  ];

  const handleSidebarToggle = () => {
    setIsSidebarMinimized(!isSidebarMinimized);
  };

  const getButtonClass = (featureName: string) =>
    `flex items-center px-3 py-2 text-sm font-medium rounded-lg transition-all ${
      selectedFeature === featureName
        ? "bg-blue-100 text-blue-600 font-semibold shadow-lg"
        : "text-gray-700 hover:bg-gray-100 hover:shadow-md"
    }`;

  return (
    <Layout
      menuData={menu}
      onMenuClick={() => router.push("/data-scraper")}
      selectedMenu="Data Scraper"
      breadcrumbItems={breadcrumbItems}
    >
      <div className="flex h-screen">
        {/* Sidebar Features */}
        <div
          className={`${
            isSidebarMinimized ? "w-16" : "w-64"
          } h-full bg-white flex flex-col border-r border-gray-200 shadow transition-all duration-300`}
        >
          {/* Header Section */}
          <div
            className={`flex items-center ${
              isSidebarMinimized ? "justify-center" : "justify-between"
            } p-4 bg-blue-100 border-b border-gray-200`}
          >
            {!isSidebarMinimized && (
              <h1 className="text-lg font-semibold text-blue-600">Data Scraper</h1>
            )}
            <button
              onClick={handleSidebarToggle}
              className="bg-white rounded-full p-2 text-blue-500 hover:text-blue-600 hover:bg-gray-100 transition-all"
              title={isSidebarMinimized ? "Expand Sidebar" : "Collapse Sidebar"}
            >
              {isSidebarMinimized ? <FaChevronRight /> : <FaChevronLeft />}
            </button>
          </div>

          {/* Feature List */}
          <ul className="flex-1 space-y-2 mt-4 px-2">
            {features.map((feature) => (
              <li key={feature.name}>
                <button
                  className={`${getButtonClass(feature.name)} ${
                    isSidebarMinimized
                      ? "justify-center flex-col h-10 w-10 mx-auto"
                      : "justify-start flex-row w-full"
                  } flex items-center hover:shadow-lg`}
                  onClick={() => setSelectedFeature(feature.name as keyof typeof subFeatures)}
                  title={isSidebarMinimized ? feature.name : undefined}
                >
                  <feature.icon
                    className={`${
                      isSidebarMinimized ? "w-5 h-5" : "w-6 h-6"
                    } text-blue-500`}
                  />
                  {!isSidebarMinimized && (
                    <span className="ml-3 text-sm">{feature.name}</span>
                  )}
                </button>
              </li>
            ))}
          </ul>
          <footer className="text-center p-4 text-gray-500 text-xs">
            Data Scraper © 2025
          </footer>
        </div>

        {/* Main Content */}
        <main className="flex-1 p-6 bg-gray-50">
          <h1 className="text-2xl font-bold mb-6 text-gray-800">{selectedFeature}</h1>
          <h2 className="text-lg font-semibold mb-4">Sub-Features:</h2>
          <ul className="list-disc list-inside space-y-2">
            {subFeatures[selectedFeature]?.map((subFeature, index) => (
              <li key={index} className="text-gray-700">
                {subFeature}
              </li>
            ))}
          </ul>
          <button className="mt-6 bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-600 transition">
            Explore {selectedFeature}
          </button>
        </main>
      </div>
    </Layout>
  );
};

export default DataScraperPage;


// import React, { useState } from "react";
// import {
//   FaSpider,
//   FaFileDownload,
//   FaSearch,
//   FaDatabase,
//   FaCogs,
//   FaChevronRight,
//   FaChevronLeft,
// } from "react-icons/fa";

// const DataScraperPage = () => {
//   const features = [
//     { name: "Web Scraping", icon: FaSpider },
//     { name: "Data Crawling", icon: FaSearch },
//     { name: "Data Export", icon: FaFileDownload },
//     { name: "Database Integration", icon: FaDatabase },
//     { name: "Advanced Options", icon: FaCogs },
//   ];

//   const subFeatures: Record<
//     | "Web Scraping"
//     | "Data Crawling"
//     | "Data Export"
//     | "Database Integration"
//     | "Advanced Options",
//     string[]
//   > = {
//     "Web Scraping": [
//       "Scrape HTML Elements",
//       "Extract Data from Tables",
//       "Scrape Images and Media",
//       "Pagination Handling",
//     ],
//     "Data Crawling": [
//       "Crawl Multiple Pages",
//       "Crawl Entire Websites",
//       "Custom URL Patterns",
//       "Rate Limiting and Throttling",
//     ],
//     "Data Export": [
//       "Export to CSV",
//       "Export to JSON",
//       "Export to XML",
//       "Custom Export Templates",
//     ],
//     "Database Integration": [
//       "Save Data to SQL Databases",
//       "Save Data to NoSQL Databases",
//       "Real-Time Database Sync",
//       "Data Backup Options",
//     ],
//     "Advanced Options": [
//       "Proxy Support",
//       "Authentication Handling",
//       "Custom Headers and Cookies",
//       "JavaScript Rendering with Puppeteer",
//     ],
//   };

//   const [selectedFeature, setSelectedFeature] = useState<keyof typeof subFeatures>(
//     "Web Scraping"
//   );
//   const [isSidebarMinimized, setIsSidebarMinimized] = useState(false);

//   const handleSidebarToggle = () => {
//     setIsSidebarMinimized(!isSidebarMinimized);
//   };

//   const getButtonClass = (featureName: string) =>
//     `flex items-center px-3 py-2 text-sm font-medium rounded-lg transition-all ${
//       selectedFeature === featureName
//         ? "bg-blue-100 text-blue-600 font-semibold shadow-lg"
//         : "text-gray-700 hover:bg-gray-100 hover:shadow-md"
//     }`;

//   return (
//     <div className="flex h-screen w-screen">
//       {/* Sidebar */}
//       <div
//         className={`bg-white ${
//           isSidebarMinimized ? "w-16" : "w-64"
//         } flex flex-col border-r border-gray-200 shadow transition-all duration-300`}
//       >
//         {/* Header Section */}
//         <div
//           className={`flex items-center ${
//             isSidebarMinimized ? "justify-center" : "justify-between"
//           } p-4 bg-blue-100 border-b border-gray-200`}
//         >
//           {!isSidebarMinimized && (
//             <h1 className="text-lg font-semibold text-blue-600">Data Scraper</h1>
//           )}
//           <button
//             onClick={handleSidebarToggle}
//             className="bg-white rounded-full p-2 text-blue-500 hover:text-blue-600 hover:bg-gray-100 transition-all"
//             title={isSidebarMinimized ? "Expand Sidebar" : "Collapse Sidebar"}
//           >
//             {isSidebarMinimized ? <FaChevronRight /> : <FaChevronLeft />}
//           </button>
//         </div>

//         {/* Menu Items */}
//         <ul className="flex-1 space-y-2 mt-4 px-2">
//           {features.map((feature) => (
//             <li key={feature.name}>
//               <button
//                 className={`${getButtonClass(feature.name)} ${
//                   isSidebarMinimized
//                     ? "justify-center flex-col h-10 w-10 mx-auto"
//                     : "justify-start flex-row w-full"
//                 } flex items-center hover:shadow-lg`}
//                 onClick={() => setSelectedFeature(feature.name as keyof typeof subFeatures)}
//                 title={isSidebarMinimized ? feature.name : undefined}
//               >
//                 <feature.icon
//                   className={`${
//                     isSidebarMinimized ? "w-5 h-5" : "w-6 h-6"
//                   } text-blue-500`}
//                 />
//                 {!isSidebarMinimized && (
//                   <span className="ml-3 text-sm">{feature.name}</span>
//                 )}
//               </button>
//             </li>
//           ))}
//         </ul>
//         <footer className="text-center p-4 text-gray-500 text-xs">
//           Data Scraper © 2025
//         </footer>
//       </div>

//       {/* Main Content */}
//       <main className="flex-1 p-6 bg-gray-50">
//         <h1 className="text-2xl font-bold mb-6 text-gray-800">{selectedFeature}</h1>
//         <h2 className="text-lg font-semibold mb-4">Sub-Features:</h2>
//         <ul className="list-disc list-inside space-y-2">
//           {subFeatures[selectedFeature]?.map((subFeature, index) => (
//             <li key={index} className="text-gray-700">
//               {subFeature}
//             </li>
//           ))}
//         </ul>
//         <button className="mt-6 bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-600 transition">
//           Explore {selectedFeature}
//         </button>
//       </main>
//     </div>
//   );
// };

// export default DataScraperPage;
