...
const CryptographyGeneratorPage = () => {
  ...
  useEffect(() => {
    if (router.pathname === "/cryptography-generator") {
      setSelectedFeature("Highlights");
    }
  }, [router.pathname]);

  const breadcrumbItems = [
    { label: "Home", href: "/", icon: <HomeIcon className="w-4 h-4" /> },
    { label: "Cryptography Generator", href: "/cryptography-generator", icon: <FolderIcon className="w-4 h-4" /> },
    ...(selectedFeature
      ? [
          {
  ...
  return (
    <Layout
      menuData={menu}
      onMenuClick={() => router.push("/cryptography-generator")}
      selectedMenu="Cryptography Generator"
