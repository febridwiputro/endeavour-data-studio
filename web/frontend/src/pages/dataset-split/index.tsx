...
const DatasetSplitPage = () => {
  const features = [
    { name: "Audio Dataset Split", icon: FaMusic },
    { name: "Text Dataset Split", icon: FaFont },
    { name: "Image Dataset Split", icon: FaImage },
  ];

  const subFeatures: Record<
    "Audio Dataset Split" | "Text Dataset Split" | "Image Dataset Split",
    string[]
  > = {
    "Audio Dataset Split": [
      "Split by Duration",
  ...
  useEffect(() => {
    if (router.pathname === "/dataset-split") {
      setSelectedFeature("Audio Dataset Split");
    }
  }, [router.pathname]);
  ...
  return (
    <Layout
      menuData={menu}
      onMenuClick={() => router.push("/dataset-split")}
      selectedMenu="Dataset Split"
...
export default DatasetSplitPage;
...
//         >
//           {!isSidebarMinimized && (
//             <h1 className="text-lg font-semibold text-blue-600">Dataset Split</h1>
//           )}
//           <button
//             onClick={handleSidebarToggle}
//             className="bg-white rounded-full p-2 text-blue-500 hover:text-blue-600 hover:bg-gray-100 transition-all"
