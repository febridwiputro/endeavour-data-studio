...
const VerifyCodePage: React.FC = () => {
  ...
  useEffect(() => {
    if (queryEmail && typeof queryEmail === "string") {
      setEmail(queryEmail); // Set email from query parameter
    } else {
      setAlert({
        show: true,
        type: "error",
        message: "Email is missing. Redirecting to sign-up.",
      });
  ...
  const handleVerifyCode = async () => {
    if (!email) {
      handleAlert("error", "Email is missing. Please log in again.");
    ...
    try {
      ...
      if (verifyCode.fulfilled.match(result)) {
        handleAlert("success", "Email verified successfully!");
        setTimeout(() => router.push("/login"), 3000);
      } else {
