...
const ImageEditorPage = () => {
  ...
  useEffect(() => {
    // Reset selected feature when navigating to the base page
    if (router.pathname === "/image-editor") {
      setSelectedFeature("cropImage");
    }
  }, [router.pathname]);

  const breadcrumbItems = [
    { label: "Home", href: "/", icon: <HomeIcon className="w-4 h-4" /> },
    { label: "Image Editor", href: "/image-editor", icon: <FolderIcon className="w-4 h-4" /> },
    ...(selectedFeature
      ? [
          {
            label: features.find((f) => f.key === selectedFeature)?.name || "",
            href: "",
  ...
  return (
    <Layout
      menuData={menu}
      onMenuClick={() => router.push("/image-editor")}
      selectedMenu="Image Editor"
    ...
    >
      <div className="flex h-screen">
        ...
        >
          ...
          >
            {!isSidebarMinimized && (
              <h1 className="text-lg font-semibold text-blue-600">Image Editor</h1>
            )}
            <button
