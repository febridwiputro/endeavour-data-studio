...
interface SidebarFiltersProps {
  ...
  onPredictionComplete: () => void; // Callback to notify parent about completed prediction
}
...
const SidebarFilters: React.FC<SidebarFiltersProps> = ({
  ...
  const handlePredict = async () => {
    if (!modelApiUrl) {
      console.error("Model API URL is not available.");
      return;
    }
    ...
    try {
      ...
      onPredictionComplete(); // Notify parent component
    } catch (error) {
...
//             placeholder="Search..."
//             className="
// w-full px-4 py-2 mb-3 border border-gray-300 rounded text-sm"
//             onChange={(e) => {
//               const searchQuery = e.target.value.toLowerCase();
//               setSelectedClasses((prev) => {
//                 const updated = { ...prev };
//                 classes.forEach((cls) => {
//                   updated[cls.name] = cls.name.toLowerCase().includes(searchQuery)
//                     ? prev[cls.name]
//                     : false;
//                 });
//                 return updated;
