...
const DataAnalyticsPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchClassData = async () => {
      if (!accessToken) return; // Ensure the accessToken is available
      setLoading(true);

      try {
        const projectId = 1; // Replace with the actual project ID
        const response = await api.get(
          `/annotations/image-annotations/class-count/?project_id=${projectId}`,
          {
            headers: {
              Authorization: `Bearer ${accessToken}`, // Include the accessToken in the request header
            },
          }
        );
        ...
        const formattedClasses = data.map((cls: any) => ({
          name: cls.label,
          count: cls.count,
          color: cls.color, // Color from the API response
        }));
