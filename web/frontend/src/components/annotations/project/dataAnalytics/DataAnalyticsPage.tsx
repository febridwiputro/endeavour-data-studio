...
const DataAnalyticsPage: React.FC = () => {
    ...
    const fetchClassData = async () => {
      ...
      try {
        const response = await api.get(
          `/annotations/image-annotations/class-count/?project_id=${selectedProjectId}`,
          {
            headers: {
  ...
  const maxCount = Math.max(...classes.map((cls) => cls.count), 1);
  ...
  // Konfigurasi data untuk histogram
  const histogramData = {
    labels: classes.map((cls) => cls.name),
    datasets: [
      {
        label: "Object Count",
        data: classes.map((cls) => cls.count),
        backgroundColor: classes.map((cls) => cls.color),
        borderColor: classes.map((cls) => cls.color),
        borderWidth: 1,
      },
    ],
  };

  // Konfigurasi opsi histogram
  const histogramOptions = {
    ...
    plugins: {
      ...
      tooltip: {
        callbacks: {
          label: (context: any) => `${context.raw} objects`,
        },
      },
    },
    scales: {
          ...
          {classes.map((cls, index) => (
            <div key={index} className="flex items-center mb-3">
              ...
              <div className="flex-grow">
                <p className="text-sm text-gray-800 font-medium">{cls.name}</p>
              ...
              {/* Count */}
              <p className="text-sm text-gray-700 ml-4 font-medium">{cls.count}</p>
            </div>
          ))}
        </div>
      )}
